...
do
    ...
    --[[ assignment statement, not a comparison check.
    It creates a local boolean flag variable and initializes its value to false.
    Initialization: "local in_error = false" sets up a guard flag before defining the error handler function.
    Event Trigger: The "awesome.connect_signal("debug::error", function (err) ... end)" block
    registers a callback function that only runs when an error actually happens.

    Loop Prevention: Inside the function, if in_error then return end checks the flag.
    If an error occurs while trying to display an error notification, in_error will be true,
    causing the function to exit immediately and prevent a crash from an endless error loop.]]--
...
terminal = "x-terminal-emulator" -- todo change this shit to a different terminal if possible without breaking stuff, idk
editor = os.getenv("EDITOR") or "nano" -- todo nano is fine, if possible maybe consider mousepad?
...
local menu_terminal = { "Open Terminal", terminal } -- make the wildcard/variable for terminal set to a real world terminal of my preferred choice
...
--awful.spawn.with_shell("feh --bg-fill /home/yournamehere/Pictures/yourpic.png")
